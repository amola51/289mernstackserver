...
import { cookieOptions, getDataUri, sendMail, sendToken } from '../utils/features.js';
...
export const login = asyncError(async(req, res,next) =>{
  ...
  export const signup = asyncError(async(req, res,next) => {
    ...
    if(req.file){
      ...
      // add cloudnary for upload image
...
export const forgetPassword = asyncError(async(req, res, next) => {
  ...
  await user.save();
  const message =`Your OTP For Resetting Passwort is ${otp}.\n Please Ignore If You Haven't requested this.`;
  try {
    await sendMail("OTP For Resetting Password", user.email, message);
  } catch (error) {
    user.otp =null,
